...
func validExchangeJSON(enrollmentToken, deviceID string, expiresAt time.Time) string {
	...
	return `{"enrollment_token":"` + enrollmentToken + `","token_type":"Bearer","expires_at":"` + expiresAt.UTC().Format(time.RFC3339Nano) + `","scope":["overlay:config:read","overlay:config:ack","overlay:device:revoke"],"device_credential":{"credential_id":"xdcid_0123456789abcdef0123456789abcdef","credential":"xdc_0123456789abcdef0123456789abcdef.AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA","token_type":"Device","issued_at":"` + issuedAt.Format(time.RFC3339) + `","expires_at":"` + credentialExpiresAt.Format(time.RFC3339) + `","scope":["overlay:session:mint","overlay:credential:rotate","overlay:device:revoke"]},"device":{"id":"` + deviceID + `","user_id":"11111111-2222-4333-8444-555555555555","network_id":"net_private","name":"Laptop","platform":"linux","hostname":"laptop","wireguard_public_key":"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=","wireguard_address":"10.77.0.10/32","created_at":"` + issuedAt.Format(time.RFC3339) + `","updated_at":"` + issuedAt.Format(time.RFC3339) + `"},"network":{"id":"net_private","display_name":"Private","cidr":"10.77.0.0/16","gateway_id":"gw-private","gateway_wireguard_public_key":"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=","gateway_endpoint_host":"gateway.example.test","gateway_endpoint_port":51820,"transport_server_name":"gateway.example.test","transport_port":443,"transport_auth_id":"11111111-1111-1111-1111-111111111111","created_at":"` + issuedAt.Format(time.RFC3339) + `","updated_at":"` + issuedAt.Format(time.RFC3339) + `"},"signing_keys":` + strings.TrimSuffix(strings.TrimPrefix(validSigningKeysJSON(), `{"keys":`), `}`) + `}`
}
